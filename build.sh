...
case "$COMMAND" in
  ...
  semver-check)
    ...
    OLD_SO="$OLD_DIR/build/liba_map_reduce_library_shared.so"
    NEW_SO="build/liba_map_reduce_library_shared.so"
  ...
  coverage)
    ...
    llvm-cov report "$COV_BUILD_DIR/liba_map_reduce_library_debug.a" \
